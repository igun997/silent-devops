...
func (s *Service) Login(ctx context.Context, source, username, password string) (*devopsv1.LoginResponse, error) {
	...
	token, err := s.issuer.Issue(id, devopsv1.Role(role), s.now())
	if err != nil {
		return nil, err
	}
